...
for(i in 1:nrow(traffic.DA)){
  if(traffic.DA[i,6]<15){
    width[i] = 1
  } else if(traffic.DA[i,6]<20&traffic.DA[i,6]>=15){
    width[i] = 2
  } else{
    width[i] = 3
  }
...
traffic.DA$width<-as.factor(traffic.DA$width)
write.csv(strtable, "strtable.csv", row.names=T)  

which(is.na(traffic.DA))
##train&test
...
train$width<-as.character(train$width)          ##這邊要先將原本的levels轉乘character，否則levels最後還是會算成3
str(train)
...
train12$width<-as.factor(train12$width)        ##再將character轉成levels，分組計算maha distance
train13$width<-as.factor(train13$width)
train23$width<-as.factor(train23$width)
...
for (i in 1:16){
  #print(i)
  a<-maha(train12[,i], groups=train12$width, method = "var")
  #print(a)
...
for (i in 1:16){
  #print(i)
  a<-maha(train13[,i], groups=train13$width, method = "var")
  #print(a)
...
for (i in 1:16){
  #print(i)
  a<-maha(train23[,i], groups=train23$width, method = "var")
  #print(a)
...
table$mahavalue12<-as.numeric(table$mahavalue12)
table$mahavalue23<-as.numeric(table$mahavalue23)
table$mahavalue13<-as.numeric(table$mahavalue13)
str(table)
table<-table[,c(2,3,4)]
library(matrixStats)
table$maha_min<-rowMins(as.matrix(table[,c(1,2,3)]))
...
size1 = sample(1:nrow(train1), 200)
size2 = sample(1:nrow(train2), 200)
size3 = sample(1:nrow(train3), 200)
...
library(MASS)
# Fit the model
model.lda <- lda(width~快車道數量, data = train.transformed)
# Make predictions
